...
$('#playPause').addEventListener('click',()=>{
 ...
 if(id==='projectile'){
  const T=projectileStats(simValues).time;
  if(running){running=false;$('#playPause').textContent='Resume flight';$('#simState').textContent='Flight paused';return;}
  if(simTime<=0||simTime>=T-.001)simTime=0;
  running=true;$('#playPause').textContent='Pause flight';$('#simState').textContent=slow?'Launching in slow motion':'Projectile in flight';
  return;
 }
